...
* When editing a foreach card, the collection attribute expression was defaulted to "payload", and that default value would remain even when trying to delete it. This has been fixed, ensuring the field can be cleaned up completely.
* Script errors are now being flagged within the transform card and all expression fields, blocking the execution of the app if present.
* The behavior supporting default connector icons has been fixed.
Several fixes around the log pane.
