...
For logging, Mule uses link:http://www.slf4j.org/[slf4j], which is a logging facade that discovers and uses a logging strategy from the classpath, such as link:https://logging.apache.org/log4j/2.x/[log4j2] or the JDK Logger. By default, Mule includes log4j2, which is configured with a file called `log4j2.xml`.
...
Mule runtimes uses log4j as the framework for logging. If you want to add or change the monitoring interval at which log4j checks for changes, you need to add the `monitorInterval` to the `log4j2.xml` configuration file.
...
----
+
This example makes log4j will check for changes every 15 seconds.
+
. Save the file, and restart the Mule server.

For more information, check for "Automatic Reconfiguration" in  link:https://logging.apache.org/log4j/2.x/manual/configuration.html[Log4j Configuration].
