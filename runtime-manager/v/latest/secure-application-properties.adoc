= Safely Hiding Mule Application Properties
...
[NOTE] 
====
Internally, a Mule application deployent's safely-hidden properties are encrypted and stored in a MuleSoft managed CloudHub properties database, which is encrypted per user organization. 
====
...
When moving applications between *sandboxes*, safely-hidden application property values will _not_ be copied to the new environment. For all safely-hidden properties, the name of the property will be copied, but the value will be left blank. 
