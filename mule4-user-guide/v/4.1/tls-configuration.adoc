...
When a TLS communication takes place between two systems, a negotiation determines which protocol and cipher suite are used. Optionally, administrators can specify the protocols and cipher suites to use, so that app developers can then specify which cipher suites and protocols to use in `tls:context`.
