...
In case you wish to migrate applications currently running in unsupported versions like 3.7.x, 3.6.x or prior, you can still use the migration tool for those version. However, a greater ratio of migration errors, unsupported patterns or incorrect code generation is to be expected. 

Although Mulesoft won't oficially support these cases, it's probably a good idea to try the tool anyway since dealing with limitations is probably still much easier than dealing with a completely unaided migration.

=== Migrating DataMapper

DataMapper is not supported by this tool. Our recommendation in this case is to first migrate your DataMapper transformations to DataWeave in your Mule 3.x first, and run the tool afterwards.
...
For future releases, we will also support the likes of:

For the GA release we will also support the likes of:
