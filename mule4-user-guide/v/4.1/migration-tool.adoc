= Migrating Mule 3 Applications to Mule 4
...
remains to migrate. The output is designed to work with a <<compatibility_module, Compatibility module>> in Studio for some items that are not migrated
...
* Mule applications only (no domains or policies supported)
