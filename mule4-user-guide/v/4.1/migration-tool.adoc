...
. If the connector has a `config` element, add a new configuration equivalent to what the application had in Mule 3.
. Migrate the sources and inbound endpoints to the source provided by the connector for Mule 4.
. Migrate the operations and outbound endpoints to the operations provided by the connector for Mule 4.
. Migrate any expressions that use the inbound properties set by a source or operation of a connector in Mule 3 to refer to the `attributes` instead.
