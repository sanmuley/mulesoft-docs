...
Expressions that use inbound attachments should use the DataWeave features for handling multipart.
...
should respectively be migrated to
...
For more in depth documentation, refer to https://docs.mulesoft.com/mule4-user-guide/v/4.1/dataweave-formats#format_form_data

Outbound Attachments

Instead of setting outbound attachments for them to be sent by the next operation in the flow or the response of the source, each operation and source must make explicit any attachments sent. These attachments to be sent may be set as variables for later reference in an expression.

Refer to the documentation of each connector for details on how to declare attachments to be sent.
