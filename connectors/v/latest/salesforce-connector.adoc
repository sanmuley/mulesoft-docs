...
The Salesforce link:http://www.salesforce.com/us/developer/docs/api_streaming/[Streaming API] securely receives notifications for changes to your organization's information in Salesforce. See <<Receive Inbound Data From Salesforce>> for more information on the use of the Streaming API.
...
| <<OAuth v2.0 Configuration, OAuth 2>> |https://help.salesforce.com/articleView?id=remoteaccess_oauth_web_server_flow.htm[OAuth 2.0 Web Server Authentication Flow]
...
** OAuth Callback Config: Listener Config, Callback Path, and Authorize Path.
...
For the Mule 4 Salesforce Connector, for all the configurations *except OAuth v2*, you have the option to keep the session alive until it expires by checking the *Disable session invalidation* checkbox.
...
See also <<Receive Inbound Data From Salesforce>> for event processing when streaming data to
