...
Yes, OSv2 will be in the same region of the worker where the app is initially deployed. For example, if you deploy to the Singapore region, the object store persists in the Singapore region. 

If after the first deploy the app is moved to a different region, then *the OSv2 will remain in the original region* in order to avoid data loss. The OSv2 will never me moved from one region to another. 

In the case you need *to deploy both app and OS in a different region*, the application should be deleted and re-uploaded, thus *losing all the data* that may exist in the storage.
